...
| | 4.A.2 | Populate production `.env` on Droplet with `DATABASE_URL`, `SECRET_KEY`, `GITHUB_TOKEN_ENCRYPTION_KEY`, `GEMINI_API_KEY`, `OPENAI_API_KEY`, `CORS_ORIGINS=https://maple-a1.com`, `APP_ENV=production`. Verify file mode `600`, owned by `maple` user. | 1.5 | 4.A.1 | **Yes** — Team Lead must supply production API keys (Gemini, OpenAI), a Fernet `GITHUB_TOKEN_ENCRYPTION_KEY`, and signed JWT `SECRET_KEY`. Instructors enter GitHub PATs in app settings. Output: secret values delivered via 1Password / secure channel. | `.env` must be gitignored; never commit. Cross-check against `.env.example`. |
