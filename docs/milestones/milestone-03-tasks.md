...
|---|---|---|---|---|---|
| 1 | LLM `complete()` retry + backoff | Jayden | `docs/design-doc.md` | §8 "2-retry-per-model logic with exponential backoff"; §4 "Retry Policy" | **done** — `server/app/services/llm.py:complete()` (lines 208–300). Verified 2026-04-22 by `pytest tests/test_llm.py tests/test_llm_extended.py tests/test_provider_dispatch.py` (54 passed). |
| 2 | Model fallback chain + timeouts | Jayden | `docs/design-doc.md` | §8 "timeouts (30s standard, 60s complex); model fallback chain"; §4 fallback spec | **done** — `MODEL_CHAIN` + `_dispatch` in `server/app/services/llm.py`; redact wiring at lines 230–231; timeouts drawn from `settings.LLM_TIMEOUT_STANDARD/COMPLEX`. |
| 3 | pgvector schema for style chunks | Jayden | `docs/design-doc.md` | §3 §II chunk metadata fields; §8 "store in pgvector" | **done** — `alembic/versions/b2c3d4e5f6a7_add_style_guide_chunks.py` + `c3d4e5f6a7b8_resize_embedding_vector_add_hnsw.py` (HNSW cosine, vector(1536)). |
| 4 | Style guide fetch + ingestion | Jayden | `docs/design-doc.md` | §8 "Fetch and ingest style guides"; §3 §II source URLs | **done** — `server/app/services/style_guide_ingester.py` (5 sources: PEP 8, Oracle Java, ts.dev, Google JS, Google C++). |
| 5 | Semantic chunking + embedding | Jayden | `docs/design-doc.md` | §8 "Chunk by semantic heading/rule block; embed with text-embedding-3-large" | **done** — `embeddings.embed_batch` (`text-embedding-3-large`, dims=1536) called from ingester. Verified 2026-04-22 by `pytest tests/test_embeddings.py` (3 passed). |
| 6 | Cosine similarity retrieval | Jayden | `docs/design-doc.md` | §8 "cosine similarity retrieval: top-5 chunks, threshold 0.75"; §4 retrieval spec | **done** — `rag_retriever.retrieve_style_chunks` (top-5, 0.75, `no_match` logging). |
| 7 | Linter execution in Docker | Jayden | `docs/design-doc.md` | §8 "Run pylint/eslint inside Docker container"; §3 §II "Static Analysis" | **done** — `linter_runner.run_linter` + `_parse_violations`; container security reuses M2 hardening (cap_drop ALL, no-new-privileges, read-only FS, network_disabled, tmpfs). Verified 2026-04-22 by `pytest tests/test_linter_runner.py tests/test_linter_extended.py` (33 passed). |
| 8 | Linter tooling in base images | Jayden | `docs/design-doc.md` | §3 §II "linters (pylint/eslint)" | **done** — `maple-python-lint:3.12` (pylint 3.2.7) + `maple-node-lint:20` (eslint 9.10.0) pinned in `sandbox_images.LINT_PROFILES`; build + smoke-test commands documented in `docs/deployment.md` "Sandbox Images — Linters" (v1.2.0, 2026-04-17). |
| 9 | JSON schemas for pass outputs | Dom | `docs/design-doc.md` | §4 output schema; RecommendationObject spec | done |
| 10 | Schema validation + repair retry | Dom | `docs/design-doc.md` | §8 "JSON schema validation; one repair retry"; §4 repair spec | done |
| 11 | Pass 1 test reconciliation | Dom | `docs/design-doc.md` | §8 "Implement Pass 1"; §4 Pass 1 prompt + spec | done |
| 12 | AST-aware code chunk extraction | Dom | `docs/design-doc.md` | §3 §II "AST-Aware Chunking strategy" | done |
| 13 | Pass 2 style review (conditional) | Dom | `docs/design-doc.md` | §8 "Implement Pass 2"; §4 Pass 2 trigger conditions + prompt | done |
| 14 | Pass 3 synthesis | Dom | `docs/design-doc.md` | §8 "Implement Pass 3"; §4 Pass 3 prompt + RecommendationObject rules | done |
| 15 | `NEEDS_HUMAN_REVIEW` flag logic | Dom | `docs/design-doc.md` | §8 "NEEDS_HUMAN_REVIEW flag"; §4 "mark criterion as NEEDS_HUMAN_REVIEW" | done |
| 16 | Pipeline orchestration (AI passes) | Dom | `docs/design-doc.md` | §8 pipeline orchestration (implicit); §2 `ai_feedback_json` field | done |
| 17 | Update `persist_evaluation_result` | Dom | `docs/design-doc.md` | §2 "Data Model — EvaluationResult" `ai_feedback_json` | done |
| 18 | `POST /submissions/{id}/review` API | Sylvie | `docs/design-doc.md` | §1 User Story 6; §8 "approve/reject AI feedback" | done |
| 19 | `GET /submissions/{id}` AI fields | Sylvie | `docs/api-spec.md` + `docs/design-doc.md` | `GET /submissions/{id}` response; §8 "display style guide version" | done |
| 20 | `review_status` model columns | Sylvie | `docs/design-doc.md` | §1 User Story 6 (instructor review before release) | done |
| 21 | Criteria scores display (Angular) | Sylvie | `docs/design-doc.md` | §8 "rubric-aligned criteria scores display"; §4 criteria_scores spec | done |
| 22 | RecommendationObject diff viewer | Sylvie | `docs/design-doc.md` | §8 "RecommendationObject diff viewer"; §4 RecommendationObject spec | done |
| 23 | Instructor approve/reject UI | Sylvie | `docs/design-doc.md` | §8 "instructor approve/reject"; §1 User Story 6 | done |
| 24 | Style guide version display | Sylvie | `docs/design-doc.md` | §8 "display style guide version used"; §3 §II version detection | done |
| 25 | Terminal statuses update (Angular) | Sylvie | `docs/design-doc.md` | §8 "EVALUATION_FAILED"; §4 `NEEDS_HUMAN_REVIEW` → `Awaiting Review` | done |
